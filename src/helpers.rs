    ...
    ) -> ul_sys::JSValueRef,
{
    unsafe {
        ...
        let jsclass = ul_sys::JSClassCreate(&jsclassdef);

        let (jsgctx, ..) = getJSContextFromView(view);

        ul_sys::JSObjectMake(jsgctx, jsclass, hook_closure)
    }
}

pub fn getJSContextFromView(view: crate::View) -> (ul_sys::JSContextRef, ul_sys::JSObjectRef) {
    unsafe {
...
pub fn set_js_object_property(view: crate::View, name: &'static str, object: ul_sys::JSObjectRef) {
    unsafe {
        let (jsgctx, jsgctx_object) = getJSContextFromView(view);

        let c_name = std::ffi::CString::new(name).unwrap();

        let propertyName = ul_sys::JSStringCreateWithUTF8CString(c_name.as_ptr());
...
pub fn evaluate_script(view: crate::View, script: &'static str) -> ul_sys::JSValueRef {
    unsafe {
        let (jsgctx, jsgctx_object) = getJSContextFromView(view);

        let script_c_str = std::ffi::CString::new(script).unwrap();

        ul_sys::JSEvaluateScript(
            jsgctx,
            ul_sys::JSStringCreateWithUTF8CString(script_c_str.as_ptr()),
            jsgctx_object,
